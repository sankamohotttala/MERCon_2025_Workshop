#this includes ML keywords as well - so this is ML + tinyML++
import os
import pandas as pd
import re
...
ml_keywords_loaded_real = pd.read_csv(r"F:\Personal\Sanka\mercon\sicet_2025_proposal_analysis\ml_keywords.csv")
ml_keywords_ = ml_keywords_loaded_real['Keyword'].tolist()

nn_keywords_loaded_real = pd.read_csv(r"F:\Personal\Sanka\mercon\sicet_2025_proposal_analysis\nn_keywords.csv")
nn_keywords_ = nn_keywords_loaded_real['Keyword'].tolist()

ml_keywords_loaded = pd.read_csv(r"F:\Personal\Sanka\mercon\sicet_2025_proposal_analysis\ml_keywords.csv")
ml_keywords = ml_keywords_loaded['Keyword'].tolist()

ml_keywords = nn_keywords_ + ml_keywords_
        ...
        if filename.endswith('.csv'):  # Check if the file is a CSV
            ...
            if doi_column and title_column:
                for _, row in df.iterrows():
                    ...
                    if matched_columns and keyward_matched_:
                        ...
                        matched_keyword_columns_ = ", ".join(matched_columns_)
                        matched_keyword_values_ = ", ".join(keyward_matched_)
                        output_data.append({
                            "DOI": doi.values[0],
                            "Title": title.values[0],
                            "Matched Columns": matched_keyword_columns,
                            "Matched keywards": matched_keyword_values,
                            "Matched Columns 2": matched_keyword_columns_,
                            "Matched keywards 2": matched_keyword_values_
                        })
                
output_df = pd.DataFrame(output_data)
output_csv_path = os.path.join(r'F:\Personal\Sanka\mercon\sicet_2025_proposal_analysis\tmp','ml_international_all.csv')
